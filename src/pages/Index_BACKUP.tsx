...
const Index = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!params.environment || params.environment.length === 0 || params.xpThreshold <= 0) {
      console.log('Validation failed - environment or xpThreshold invalid');
      toast({
      ...
      if (errorMessage.includes('No creatures found for environment') && errorMessage.includes('Available environments:')) {
        ...
        toast({
          ...
          description: <ErrorToastContent
            title="No Creatures Found"
            message={`No creatures available for "${params.environment.join(', ')}". Available: ${availableEnvs}`}
            error={detailedError}
          />,
          variant: "destructive"
        });
  ...
  const handleGenerateMagicItems = async (rarities: string[], maxItems: number) => {
    ...
    console.log('Proceeding with magic item generation');
    ...
    try {
      ...
      // Fetch magic items for all selected rarities
      let allItems: any[] = [];

      if (rarities.length === 0) {
        // Fetch all items if no rarity specified
      ...
      const limited = shuffled.slice(0, maxItems);

      setMagicItems(limited);
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="text-center cursor-help">
                              ...
                              <div className="font-semibold text-accent">{params.environment.join(', ')}</div>
                            </div>
                          </TooltipTrigger>
                          <TooltipContent>
            ...
            {magicItems.length > 0 && (
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <Sparkles className="h-6 w-6 lg:h-7 lg:w-7 text-accent" />
